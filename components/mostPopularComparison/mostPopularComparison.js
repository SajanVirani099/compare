...
const MostPopularComparison = ({ popularComparison = [] }) => {
    ...
    return (
        <div className="max-w-[1280px] w-full mx-auto my-6 sm:my-8 px-4 sm:px-6 md:px-8 lg:px-0 border-[#d1d9e6] border-2 rounded-lg">
            ...
            <div className="flex items-center justify-between gap-4 border-b border-[#d1d9e6] py-3 px-2 lg:px-3">
                {/* Left Navigation Arrow - Neumorphic Up Theme */}
                <button
                    onClick={() => scroll("left")}
                    disabled={!canScrollLeft}
                    className={`sm:p-0 w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-[#E6E7EE] shadow-[3px_3px_6px_#d1d9e6,-3px_-3px_6px_#ffffff] flex items-center justify-center transition-all duration-200 ${
                        !canScrollLeft
                ...
                >
                    <FaChevronLeft className="w-4 h-4 text-[#434343]" />
                </button>
                ...
                >
                    <FaChevronRight className="w-4 h-4 text-[#434343]" />
                </button>
            </div>
                ...
                >
                    {pairs.map((pair, index) => {
                        ...
                        return (
                            <div
                                key={index}
                                className="flex-shrink-0 w-[250px] sm:w-[280px] md:w-[300px] cursor-pointer"
                                onClick={() => {
                                    router.push(`/compare/${left?.uniqueTitle},${right?.uniqueTitle}`)
                                }}
                            >
                                {/* Card - Neumorphic Up Theme */}
                                <div className="rounded-xl bg-[#E6E7EE] shadow-[3px_3px_6px_#d1d9e6,-3px_-3px_6px_#ffffff] p-2 sm:p-3">
                                    {/* Top Section - Product Images with VS Badge */}
                                    <div className="relative flex items-center justify-center gap-2 sm:gap-3 mb-4">
                                        {/* Left Product Image */}
                                        {left && (
                                            <div className="w-28 h-28 sm:w-32 sm:h-32 md:w-36 md:h-36 rounded-lg bg-white shadow-[inset_2px_2px_4px_#d1d9e6,inset_-2px_-2px_4px_#ffffff] p-1.5 sm:p-2 flex items-center justify-center overflow-hidden">
                                                <img
                                                    src={leftImg}
                                                    alt={leftName}
                                    ...
                                    <div className="flex flex-col items-center justify-center gap-2 text-center">
                                        {/* First Product Name */}
                                        <p className="text-xs font-medium text-[#434343] w-full truncate px-2">
                                            {leftName}
                                        </p>
                                        ...
                                        {right && (
                                            <div className="relative w-full flex items-center justify-center">
                                                ...
                                                <div className="relative bg-[#E6E7EE] shadow-[inset_2px_2px_4px_#d1d9e6,inset_-2px_-2px_4px_#ffffff] rounded-full px-2.5 py-1.5 font-bold text-[#434343] flex items-center justify-center w-2 h-2 sm:w-4 sm:h-4 md:w-6 md:h-6 before:content-[''] before:absolute before:h-[1px] before:w-[60px] sm:before:w-[40px] md:before:w-[60px] before:bg-[#d1d9e6] before:left-[-65px] sm:before:left-[-45px] md:before:left-[-65px] before:top-1/2 before:-translate-y-1/2 after:content-[''] after:absolute after:h-[1px] after:w-[60px] sm:after:w-[40px] md:after:w-[60px] after:bg-[#d1d9e6] after:right-[-65px] sm:after:right-[-45px] md:after:right-[-65px] after:top-1/2 after:-translate-y-1/2">
